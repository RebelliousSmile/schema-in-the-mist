...
export type ReleaseTrainManifest = {
  version: 1;
  provider: { commit: string; finalTag: string; archiveUrl: string; sha256: string };
...
export function parseReleaseTrainManifest(value: unknown): ReleaseTrainManifest {
  ...
  exactKeys(provider, ["commit", "finalTag", "archiveUrl", "sha256"], "provider");
  ...
  return { version: 1, provider: { commit: providerCommit, finalTag, archiveUrl, sha256 }, consumers: { lantern: { commit: lanternCommit }, handbook: { commit: handbookCommit } } };
}
